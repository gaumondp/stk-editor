		...
		>
			...
			<div class="body">
				...
				<ul class="list presets">
					<li><span>{tr('sd_reader.preset_audio_drum')}</span><span class="num">{formatNumber(report.presets.audio_drum)}</span></li>
					<li><span>{tr('sd_reader.preset_audio_inst')}</span><span class="num">{formatNumber(report.presets.audio_inst)}</span></li>
...
<style>
	...
	.body { padding: 8px 0 4px; overflow-y: auto; }
	.status { margin: 8px 20px; }
	.badge { padding: 3px 10px; border-radius: 10px; font-weight: 600; font-size: 11px; color: #fff; }
	.badge.ok { background: var(--ok, #4caf50); }
	.badge.warn { background: var(--warn, #f0ad4e); color: var(--bg, #1a1d21); }
	.badge.err { background: var(--err, #e74c3c); }
	h3 { margin: 12px 20px 6px; font-size: 12px; text-transform: uppercase; letter-spacing: 0.06em; color: var(--fg-dim); }
	h3 .count { text-transform: none; letter-spacing: 0; color: var(--fg-dim); font-weight: 400; }
	.path { margin: 0 20px; font-size: 12px; word-break: break-all; color: var(--fg); }
	.empty { margin: 0 20px; font-size: 12px; color: var(--fg-dim); }
	.list { list-style: none; padding: 0 20px; margin: 0; font-size: 12px; color: var(--fg); }
	.list.warn li { color: var(--warn, #f0ad4e); }
	.list.presets li { display: flex; justify-content: space-between; max-width: 240px; padding: 2px 0; }
	.num { font-variant-numeric: tabular-nums; color: var(--fg-dim); }
	.search { margin: 0 20px; width: calc(100% - 40px); box-sizing: border-box; padding: 6px 8px; background: var(--bg, #1a1d21); border: 1px solid var(--line, #3a3f45); border-radius: 6px; color: var(--fg); font-size: 12px; }
	.audio { width: calc(100% - 40px); margin: 8px 20px 0; border-collapse: collapse; font-size: 11px; }
	.audio th, .audio td { padding: 4px 6px; border-bottom: 1px solid var(--line, #3a3f45); text-align: left; }
	.audio th { color: var(--fg-dim); font-weight: 600; }
	.audio .right { text-align: right; }
	.audio .rel { max-width: 480px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
	.footer { display: flex; justify-content: space-between; gap: 8px; padding: 12px 20px; border-top: 1px solid var(--line, #3a3f45); }
	.btn { padding: 6px 12px; border-radius: 6px; border: 1px solid var(--line, #3a3f45); background: var(--bg-3, #2e3338); color: var(--fg); cursor: pointer; }
	.btn.primary { background: var(--accent, #d2a83f); color: var(--bg, #1a1d21); border-color: var(--accent, #d2a83f); font-weight: 600; }
</style>
