...
<script lang="ts">
	...
	let missing = $state<Array<{ pad: number; file_name: string; original_path: string; sha256: string | null; candidates: string[]; loading: boolean }>>([]);
			...
			<ul class="list">
				{#each missing as item (item.pad)}
					<li class="row">
						...
						{/if}
						<button class="btn small" type="button" onclick={() => pickManual(item.pad)}>{tr('missing.choose_file')}</button>
					</li>
				{/each}
			</ul>
			<div class="actions">
...
<style>
	.backdrop { position: fixed; inset: 0; background: rgba(0,0,0,.6); display: flex; align-items: center; justify-content: center; z-index: 220; }
	.modal { background: var(--bg-2, #23272b); border: 1px solid var(--line, #3a3f45); border-radius: 12px; padding: 18px; min-width: 520px; max-width: 720px; max-height: 80vh; overflow: auto; box-shadow: 0 8px 32px rgba(0,0,0,.5); }
	h2 { margin: 0 0 8px; font-size: 16px; }
	.hint { font-size: 12px; color: var(--fg-dim, #9aa0a6); margin: 6px 0; }
	.list { list-style: none; padding: 0; margin: 8px 0; display: flex; flex-direction: column; gap: 10px; }
	.row { border: 1px solid var(--line, #3a3f45); border-radius: 8px; padding: 8px 10px; background: var(--bg, #1a1d21); }
	.head { display: flex; gap: 8px; align-items: baseline; flex-wrap: wrap; }
	.pad { font-weight: 600; color: var(--accent, #d2a83f); min-width: 60px; }
	.file { font-weight: 600; }
	.orig { font-size: 11px; color: #e74c3c; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; flex: 1; }
	.cands { list-style: none; padding: 4px 0 0; margin: 0; display: flex; flex-direction: column; gap: 4px; max-height: 120px; overflow: auto; }
	.cand { width: 100%; text-align: left; background: var(--bg-3, #2e3338); border: 1px solid var(--line); border-radius: 4px; padding: 4px 8px; font-size: 11px; cursor: pointer; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
	.cand:hover { background: #39414b; }
	...
	.btn.small { padding: 2px 8px; font-size: 11px; margin-top: 6px; }
</style>
