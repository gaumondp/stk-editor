...
<script lang="ts">
	...
	const recentProjects = $derived(
		$recentStore.entries.filter((project) => project.path.trim().length > 0 && !project.missing).slice(0, 5)
	);
			...
			{#each recentProjects as project}
				<li>
					<button
						...
						type="button"
						title={project.path}
						onclick={() => void doOpenRecent(project.path)}>{recentProjectLabel(project.path, project.name)}</button
					>
				</li>
			{/each}
		</ul>
