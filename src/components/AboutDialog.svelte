...
<style>
	...
	.modal {
		background: var(--bg-2, #23272b); border: 1px solid var(--line, #3a3f45);
		border-radius: 12px; padding: 22px; min-width: 380px; max-width: 520px;
		box-shadow: 0 8px 32px rgba(0,0,0,.5);
	}
	h2 { margin: 0 0 6px; font-size: 18px; }
	.ver { color: var(--fg-dim, #9aa0a6); font-size: 12px; margin: 0 0 2px; font-family: monospace; }
	.build { color: var(--fg-dim, #9aa0a6); font-size: 12px; margin: 0 0 12px; font-family: monospace; }
	.body { font-size: 13px; line-height: 1.5; color: var(--fg, #e6e6e6); }
	.developed-by { font-size: 12px; color: var(--fg, #e6e6e6); margin: 8px 0 0; }
	.meta { font-size: 11px; color: var(--fg-dim, #9aa0a6); margin: 12px 0 0; padding-left: 16px; }
	.diagnostics { display: flex; align-items: center; gap: 10px; margin-top: 16px; flex-wrap: wrap; }
	.hint { font-size: 11px; }
	.hint.ok { color: var(--ok, #4caf50); }
	.hint.err { color: var(--err, #e74c3c); }
	.actions { display: flex; justify-content: flex-end; margin-top: 16px; }
	.btn { padding: 6px 14px; border-radius: 6px; border: 1px solid var(--line); background: var(--bg-3, #2e3338); color: var(--fg); cursor: pointer; }
