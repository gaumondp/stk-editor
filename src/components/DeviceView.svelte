...
<script lang="ts">
	...
	const trackRows = [
		...
		[
			...
			{ id: 9, label: 'TRK 9' },
			{ id: 10, label: 'TRK 10' },
			{ id: 11, label: 'GLOBAL 1' },
			{ id: 12, label: 'GLOBAL 2' },
			{ id: 13, label: 'GLOBAL 3' },
			{ id: 14, label: 'USB AUDIO' },
			{ id: 15, label: 'EXT SRC' }
		]
	];
	...
	onMount(() => {
		...
		void import('@tauri-apps/api/window')
			.then(({ getCurrentWindow }) =>
				getCurrentWindow().onDragDropEvent((event) => handleNativeDragDrop(event.payload))
			)
		...
		startPointerDrag(event, {
			onMove: (moveEvent) => {
				...
				window.dispatchEvent(
					new CustomEvent('smpltrek-assigned-pad-drag-move', {
						detail: { sourcePad, clientX: moveEvent.clientX, clientY: moveEvent.clientY }
					})
				);
			},
			onEnd: (upEvent) => {
				suppressPadClickUntil = performance.now() + CLICK_SUPPRESSION_MS;
				window.dispatchEvent(
					new CustomEvent('smpltrek-assigned-pad-drag-end', {
						detail: { sourcePad, clientX: upEvent.clientX, clientY: upEvent.clientY }
					})
				);
			},
			onCancel: () => {
				window.dispatchEvent(new CustomEvent('smpltrek-assigned-pad-drag-cancel'));
			}
		});
	}
	...
	async function handlePadDrop(id: number, event: DragEvent | null, droppedPath?: string) {
		...
		if (data?.startsWith('file://')) {
			try {
				data = decodeURI(new URL(data.split('\n')[0].trim()).pathname);
			} catch {}
		}
		...
		if (existing) {
			...
			if (isCustom) {
				try {
					...
					const ok = await ask(tr('pad.replace_body', { pad: id, volume: existing.volume, pan: existing.pan }), {
						title: tr('pad.replace_title'),
						kind: 'warning'
					});
					if (!ok) return;
				} catch {}
		...
		<section class="assignment-panel" aria-label={tr('pad.assignments_title')}>
			<div class="assignment-heading">
				...
				<span class="assignment-count"
					>{tr('pad.assigned_count', { count: assignablePadIds.filter((id) => !!$project.kit.pads[id]).length })}</span
				>
			</div>
			<div class="assignment-list">
				{#each assignablePadIds as id}
					...
					<div
						...
						role="button"
						tabindex="0"
						aria-label={`Pad ${id}: ${sample ? sample.file_name : tr('pad.ready')}`}
						onmousedown={(event) => {
							if (sample) handleAssignedPadMouseDown(id, event);
						}}
					...
					>
						...
						{#if sample}
							<button
								...
								type="button"
								aria-label={tr('pad.remove', { pad: id })}
								title={tr('pad.remove', { pad: id })}
								onmousedown={(event) => event.stopPropagation()}
								onclick={(event) => {
									event.stopPropagation();
									handleAssignedSampleRemove(id);
								}}>×</button
							>
						{:else}
		...
		<section class="physical-pad-section" aria-label="Physical pads">
			<div class="physical-pad-layout">
				<div class="pad-actions">
					...
					<button
						class="pad-action mute"
						class:active={$previewMuted}
						type="button"
						onclick={togglePreviewMute}
						aria-pressed={$previewMuted}
						aria-label={tr($previewMuted ? 'pad.unmute_preview' : 'pad.mute_preview')}
						title={tr($previewMuted ? 'pad.unmute_preview' : 'pad.mute_preview')}
					>
						<span aria-hidden="true">{$previewMuted ? '🔇' : '🔊'}</span>
						<span>{tr($previewMuted ? 'pad.unmute_preview' : 'pad.mute_preview')}</span>
					</button>
					<button
						class="pad-action suggestions"
						type="button"
						onclick={toggleSuggestedPadPositions}
						aria-pressed={showSuggestedPadPositions}
					>
					...
					<g class="track-grid">
						{#each trackRows as row, rowIndex}
							{#each row as pad, index}
								...
								<g
									class="track-pad"
									class:assigned={!!$project.kit.pads[pad.id]}
									class:selected={selectPad === pad.id}
									class:dragover={dragOverPad === pad.id}
									class:drag-source={dragSourcePad === pad.id}
									class:previewing={previewingPad === pad.id}
									data-pad={pad.id}
									role="button"
									tabindex="-1"
									aria-label={`Pad ${pad.id}: ${pad.label}`}
									onmousedown={(event) => {
										if ($project.kit.pads[pad.id]) handleAssignedPadMouseDown(pad.id, event);
									}}
									onclick={(event) => handlePadClick(pad.id, event)}
									ondblclick={() => handlePadDoubleClick(pad.id)}
									onkeydown={(event) => handlePadKeyDown(pad.id, event)}
								>
									<text {x} y={labelY} text-anchor="middle">{pad.label}</text><rect
										x={x - 28}
										y={padY}
										width="56"
										height="58"
								...
								{#each trackRows as row, rowIndex}
									{#each row as pad, index}
										...
										<text
											...
											{x}
											y={rowIndex === 0 ? 105 : 205}
											text-anchor="middle"
											aria-label={tr(`pad.suggestion_${pad.id}`)}
											><title>{tr(`pad.suggestion_${pad.id}`)}</title>{tr(`pad.suggestion_${pad.id}_short`)}</text
										>
									{/each}
								{/each}
							</g>
...
<style>
	...
	:global(html[data-ui-scale='200']) .device {
	...
	.assignment-target {
		...
		min-height: 34px;
		padding: 5px 7px;
		border: 1px dashed var(--line, #3a3f45);
		border-radius: 5px;
	...
	.assignment-target:hover,
	.assignment-target.dragover {
		border-color: var(--accent-2, #4a90d2);
		background: rgba(74, 144, 210, 0.15);
	}
	.assignment-target.assigned {
		border-style: solid;
		border-color: var(--ok, #4caf50);
		background: rgba(76, 175, 80, 0.12);
	}
	.assignment-target.selected {
		outline: 2px solid var(--accent, #d2a83f);
		outline-offset: 1px;
	}
	.assignment-target.drag-source {
	...
	.pad-action.mute {
		...
		justify-content: center;
		gap: 4px;
		height: calc(3 * 1.2em + 14px);
		line-height: 1.2;
	}
	.pad-action.mute.active {
		border-color: var(--warn, #f0ad4e);
		background: rgba(240, 173, 78, 0.15);
	}
	.pad-action.suggestions {
		line-height: 1.2;
	}
	.pad-action.sd-reader-action {
		display: flex;
		align-items: center;
		justify-content: center;
		gap: 6px;
	...
	:global(html[data-ui-scale='125']) .physical-pad-layout,
	:global(html[data-ui-scale='150']) .physical-pad-layout,
	:global(html[data-ui-scale='200']) .physical-pad-layout {
		flex-direction: column;
	}
	:global(html[data-ui-scale='125']) .device-wrap,
	:global(html[data-ui-scale='150']) .device-wrap,
	:global(html[data-ui-scale='200']) .device-wrap {
		min-width: 680px;
	}
	:global(html[data-ui-scale='125']) .pad-actions,
	:global(html[data-ui-scale='150']) .pad-actions,
	:global(html[data-ui-scale='200']) .pad-actions {
		order: 1;
		flex: 0 0 auto;
		flex-direction: row;
		width: 100%;
	}
	:global(html[data-ui-scale='125']) .pad-action.sd-reader-action,
	:global(html[data-ui-scale='150']) .pad-action.sd-reader-action,
	:global(html[data-ui-scale='200']) .pad-action.sd-reader-action {
		margin-top: 0;
		margin-left: 24px;
	}
	.pads-svg {
	...
	.track-grid .pad-number {
		fill: var(--fg, #e6e6e6);
		font-size: 28px;
		opacity: 0.11;
		pointer-events: none;
	}
	.track-grid .pad-suggestion {
		fill: var(--fg, #e6e6e6);
		font-size: 10px;
		font-weight: 600;
		pointer-events: none;
	}
	.track-pad.assigned .pad-number,
	.track-pad.dragover .pad-number {
		fill: var(--bg, #1a1d21);
	}
	.track-pad {
		cursor: pointer;
	...
	.track-pad rect {
		fill: var(--bg-3, #2e3338);
		stroke: var(--line, #3a3f45);
		stroke-width: 2;
	}
	.track-pad.assigned rect {
		fill: var(--ok, #4caf50);
		stroke: var(--ok, #4caf50);
	}
	.track-pad.assigned text {
		fill: var(--ok, #4caf50);
	}
	.track-pad.dragover rect {
		fill: var(--accent-2, #4a90d2);
		stroke: var(--accent-2, #4a90d2);
		stroke-width: 4;
	}
	.track-pad.drag-source rect {
		stroke: var(--accent, #d2a83f);
		stroke-dasharray: 5 3;
	}
	.track-pad.dragover text {
		fill: var(--accent-2, #4a90d2);
	}
	.track-pad.selected rect {
		stroke: var(--accent, #d2a83f);
		stroke-width: 4;
	}
	.track-pad.previewing rect {
